## Version: v0.5.1
## Date: 2021-04-17
## Update Content: 修改终端默认设置和提前运行的说明
...
AutoAddThirdpardCron="false"

## 是否自动删除 thirdpard 类脚本（其他开发者脚本）的cron任务（选填）
## 本shell脚本不一定能完全从js脚本中识别出有效的cron设置，如果发现不能满足你的需要，请设置为 "false" 以取消自动删除。
AutoDelThirdpardCron="false"


## ---------------------------------- 手 动 填 写 互 助 码 区 域 （不推荐！） ----------------------------------
## 现不推荐手动填写互助码，需要手动填写的用户请将config/sharecode.sh.sample复制到config/sharecode.sh。去除不需要手动助力的地方。
